pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                cleanWs()
                checkout scmGit(branches: [[name: '*/upstream-release']], extensions: [], userRemoteConfigs: [[credentialsId: 'jenkins-github-token-as-password', url: 'https://github.com/OpenClinica/kobocat.git']])
            }
        }
        ...
        stage('Deploy Helm Chart') {
            steps {
                sh "https_proxy=socks5://127.0.0.1:1081 /usr/local/bin/helm upgrade formdesigner --install apps/kobo_kpi --namespace ${ns} --set kobocat.image.repository=${registry} --set kobocat.image.tag=$BUILD_NUMBER"
            }
        }
